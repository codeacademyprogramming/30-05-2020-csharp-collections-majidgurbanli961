...
namespace HomeworkCollection
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //FirstExercise(debtors);
            //SecondExercise(debtors);
            // ThirdExercise(debtors);
            //  FourthClass.Fourth(debtors);
            Twelve.TwelveExercise(debtors);
        ...
        static List<Debtor> FirstExercise(List<Debtor> debtors)
        {

            var resultList = debtors.Where<Debtor>(x => x.Email.Substring(x.Email.IndexOf('@')) == "@rhyta.com").ToList();
            var resultListSecond = debtors.Where<Debtor>(x => x.Email.Substring(x.Email.IndexOf('@')) == "@dayrep.com").ToList();
        ...
        static List<Debtor> SecondExercise(List<Debtor> debtors)
        {
            Debtor myDebtor = debtors[4];

            var result = debtors.Where<Debtor>(x => ageCount(x.BirthDay) >= 26 && ageCount(x.BirthDay) <= 36).ToList();
        ...
        static double ageCount(DateTime birthday)
        {
        ...
        static List<Debtor> ThirdExercise(List<Debtor> debtors)
        {
            var result = debtors.Where<Debtor>(x => x.Debt <= 5000).ToList();
            foreach (var item in result)
            {
